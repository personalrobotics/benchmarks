...
bool CollisionCheckingBenchmark::RunBenchmark(std::ostream &out, std::istream &in){
    ...
    if(doc["body"]){
        _body_name = doc["body"].as<std::string>();
    ...
    if(doc["datafile"]){
        _datafile = doc["datafile"].as<std::string>();
    }

    if(doc["extent"]){
        double extent = doc["extent"].as<double>();
...
std::vector<OpenRAVE::Transform> CollisionCheckingBenchmark::GenerateCollisionData() const {
    ...
    if(_datafile.length() == 0){
        ...
        YAML::Node doc = YAML::Load(in_stream);

        YAML::Node data_node = doc["data"];
        std::vector<CollisionData> cdata(data_node.size());
        for(unsigned int idx=0; idx < cdata.size(); idx++){
            cdata[idx] = data_node[idx].as<CollisionData>();
        }
...
std::vector<std::vector<double> > CollisionCheckingBenchmark::GenerateSelfCollisionData() const {
    ...
    if(_datafile.length() == 0){
        ...
        YAML::Node data_node = doc["data"];
        std::vector<SelfCollisionData> cdata(data_node.size());
        for(unsigned int idx=0; idx < cdata.size(); idx++){
            cdata[idx] = data_node[idx].as<SelfCollisionData>();
        }
